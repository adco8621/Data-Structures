...
Node* BST::deleteNode(Node *currNode, int value)
{
  ...
  else
  {
    ...
    {
      Node *temp = currNode, *crawl = currNode->right, *temp2;
...
void BST::isValidHelper(Node *node, int array[], int &count){
    if (node){
        isValidHelper(node->left, array, count);
        array[count] = node->key;
        count++;
        isValidHelper(node->right, array, count);
    }
}
